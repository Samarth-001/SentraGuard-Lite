...
def test_finds_email():
    ...
    # Evidence is structured ("type=email count=1 spans=[...]"), not a
    # free-text phrase, so check the structured field instead of a literal.
    assert any(e.startswith("type=email") for e in result.evidence)
...
def test_finds_phone_number():
    ...
    assert result.matched is True
    assert any(e.startswith("type=phone") for e in result.evidence)
...
def test_email_and_phone_both_redacted_in_same_text():
    ...
    types = {e.split(" ", 1)[0] for e in result.evidence}
    assert types == {"type=email", "type=phone"}
